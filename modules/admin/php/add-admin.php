...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(!empty($image)){
        move_uploaded_file($_FILES['photo']['tmp_name'], '../assets/img/admin/'.$image);
    }else{
    ...
    elseif(!preg_match("#[a-z]+#",$_POST["password"])) {
        ...
        $admin_id = substr($admin,0,3).date('y').rand(0000,9999);
        ...
        if ($mailExits->fetch(PDO::FETCH_ASSOC)) {
            ...
            $_SESSION['student_id'] = $admin_id;
            $_SESSION['username'] = $username;

            try {
                //Insert user Values and Display success Message for registration details
                $insert = "INSERT INTO admin(title, admin_id, gender, firstname, lastname, username, email, password,  address,  country, dob, phone, image, registered_on) 
                        VALUES (:title, :admin_id, :gender, :firstname, :lastname, :username, :email, :password, :address, :country, :dob, :phone, :image, :registered_on)";
